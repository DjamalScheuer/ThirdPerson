...
namespace JUTPS.CharacterBrain
{
    ...
    public class JUCharacterBrain : MonoBehaviour
    {
        ...
        public void InAirMovementControl(float JumpingControlForce, bool JumpInert = true)
        {
            if (IsGrounded)
            {
                if (JumpInert)
            ...
            else
            {
                ...
                if (IsMoving)
                {
                    rb.AddForce(DirectionTransform.forward * JumpingControlForce * 10, ForceMode.Force);
                }
            }
        }
        ...
        {
            if (IsGrounded == false || IsJumping == true || IsRolling == true || IsDriving == true || CanJump == false || IsProne)
            {
                _GetUp();
        ...
        public virtual void _NewJumpDelay(float Delay = 0.3f)
        {
            //New Jump Delay
            if (CanJump == false && IsJumping == false && IsGrounded == true && IsInvoking("_enableCanJump") == false)
            {
                VelocityMultiplier = VelocityMultiplier / 4;
